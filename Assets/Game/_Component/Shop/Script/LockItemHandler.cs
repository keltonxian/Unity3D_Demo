...
public class LockItemHandler : MonoBehaviour {
	...
	public void IapUnlock (LockManager.IAP_TYPE iapType, UnlockAnimCallbackWithItem callback=null) {
		if (iapType == LockManager.IAP_TYPE.FULL) {
			if (LockManager.Instance.IsFullUnlocked ()) {
				...
				if (null != _arrayLevelsLockItem && _arrayLevelsLockItem.Length > 0) {
	...
	private void CheckHideShop () {
		...
		bool isHide = LockManager.Instance.IsAllUnlock ();
		if (false == isHide) {
			return;
		}
...
//		}
//		if (hasMusic) {
//			AudioManager.Instance.PlaySoundEffect (Resources.Load<AudioClip> ("UnlockSoundEffect"));
//		}
		Sequence seq = DOTween.Sequence();
