...
public class ResManager : MonoBehaviour
{
	...
	//加载完成的
	private static Dictionary<string,Asset> m_LoadedKV = new Dictionary<string, Asset> ();
	//加载失败的
	private static Dictionary<string,Asset> m_LoadFailedKV = new Dictionary<string, Asset>();
	//正在加载的对象
	private static Dictionary<string,Asset> m_LoadingAssets = new Dictionary<string, Asset>();
	...
	private string GetPath(Asset asset){
		if(asset.path== AssetPath.StreamingAssets) return streamingAssetPath+(asset.useSearchPath?this.searchPath:"");
		if(asset.path== AssetPath.PersistentData) return persistentDataPath+(asset.useSearchPath?this.searchPath:"");
				...
				if (asset.path != AssetPath.Resources) {
					...
					if (www.error == null || www.error.Length == 0) {
						...
						if (asset.type == AssetType.Sprite) {
							...
							asset.audioClip = www.GetAudioClip();
	...
	public void LoadSpriteFromStreamingAsset(string url,Action<Sprite> onLoaded,Vector2 pivot,bool isAsyncInIOS = true, Action<string> onLoadFailed = null,bool useSearchPath = true)
	{
		LoadTexture2DFromStreamingAsset (url, delegate(Texture2D t) {
			Sprite s = Sprite.Create(t,new Rect(0,0,t.width,t.height),pivot,100,0,SpriteMeshType.FullRect);
	...
	public void LoadTexture2DFromStreamingAsset(string url,Action<Texture2D> onLoaded,bool isAsyncInIOS = true,bool readOnly = true, Action<string> onLoadFailed = null,bool useSearchPath = true){
		...
		#if UNITY_IOS
		string realPath = Application.streamingAssetsPath + "/" + searchPath + url;
		if(isAsyncInIOS)
		{
			StartCoroutine(WWWSyncLoadTexture2D(streamingAssetPath+ searchPath +url,onLoaded,readOnly,onLoadFailed));
		}
		else
		{
